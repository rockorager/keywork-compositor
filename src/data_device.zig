...
pub fn deinit(self: *Self) void {
    self.seat.clearKeyboardFocusListener();
    self.global.destroy();
...
fn setSelection(self: *Self, source_id: ?SourceId, serial: u32) void {
    if (self.selection != null and serialIsOlder(serial, self.selection_serial)) return;
    const old_source = self.selection;
    if (std.meta.eql(old_source, source_id)) return;
